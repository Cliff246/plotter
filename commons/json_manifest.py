...
class Manifest:
	...
	def create_subfolders(self):
		...
		plots_base = os.path.join(self.dest, "plots")
		data_base = os.path.join(self.dest, "data")
	...
	def fix_output_paths(self):
		...
		for plot in self.plots:
			...
			plot.manifest_info["output"] = os.path.join(self.dest, "plots", "Singles", plot_basename)

		# Fix plots in groups
		for group_key, group in self.groups.items():
			for plot in group:
				plot_basename = os.path.basename(plot.manifest_info["output"])  # e.g., "weight0.png"
				plot.manifest_info["output"] = os.path.join(self.dest, "plots", group_key, plot_basename)
	...
	def fix_data_paths(self):
		...
		for plot in self.plots:
			...
			new_path = os.path.join(self.dest, "data", "Singles", data_basename)
		...
		for group_key, group in self.groups.items():
			for plot in group:
				data_basename = os.path.basename(plot.manifest_info["source"])
				new_path = os.path.join(self.dest, "data", group_key, data_basename)
				plot.manifest_info["source"] = new_path
				plot.path = new_path
